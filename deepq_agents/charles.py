...
class DeepQAgent(Goshi):
    def __init__(self):
        super().__init__(f'Charles')
        self.model = self._init_model()

        # Load the model weights
        try:
            self.model.load_weights(f'./deepq_agents/DeepQ_{DIMENSION}x{DIMENSION}_charles.weights.h5')
        except:
            print("No weights found, creating new file...")
            self.model.save_weights(f'./deepq_agents/DeepQ_{DIMENSION}x{DIMENSION}_charles.weights.h5')
    ...
    def decide(self, goban: 'Goban', kakunin) -> Optional[Ten]:
        ...
        if self.last_state is not None:
            ...
            # Score is the difference between the current kakunin and the last kakunin. This means that if we are disqualified we automatically get a score of -1/0, and that captures are treated as a small reward. 
