...
class DeepQAgent(Goshi):
    def __init__(self):
        super().__init__(f'Betty')
        self.model = self._init_model()

        # Load the model weights
        try:
            self.model.load_weights(f'./deepq_agents/DeepQ_{DIMENSION}x{DIMENSION}_betty.weights.h5')
        except:
            print("No weights found, creating new file...")
            self.model.save_weights(f'./deepq_agents/DeepQ_{DIMENSION}x{DIMENSION}_betty.weights.h5')
