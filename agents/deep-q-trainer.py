...
class DeepQAgent(Goshi):
    def __init__(self):
        super().__init__(f'DeepQ')
        self.model = self._init_model()

        # Get the directory of the current file
        dir_path = os.path.dirname(os.path.realpath(__file__))

        # Construct the path to the weights file and save it as an instance attribute
        self.weights_path = os.path.join(dir_path, f'DeepQ_{DIMENSION}x{DIMENSION}.weights.h5')
